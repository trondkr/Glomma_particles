...
class SedimentDrift3D(OpenDrift3DSimulation):  # based on OpenDrift3DSimulation base class
    ...
    def horizontal_diffusion(self, *args, **kwargs):
        ...
        * as this model the same process, only with a different approach
        ...
        logging.debug('\t\t%s   <- horizontal diffusion distance [m] ->   %s' % (
        np.min(diff_velocity * self.time_step.seconds), np.max(diff_velocity * self.time_step.seconds)))
...
def bedshearstress_cw(self, p, time=None, imax=2):
    ...
    tau_cur = np.tile(0.0, np.size(p, 0))  # allocate
    tau_cw = np.tile(0.0, np.size(p, 0))  # allocate
    tau_max = np.tile(0.0, np.size(p, 0))  # allocate
    ...
    for mover in self.movers[0:]:
        if mover.topo:  # topo needed to define bedshear stress
            ...
            if (not mover.is3d) and (mover.z0 > 0):  # mover is a 2D-depth averaged current
                ...
                Cdrag = (0.4 / (np.log(abs(topo[:, 0] / mover.z0)) - 1)) ** 2
    ...
    if len(self.reactors) > 0:  # check if wave forcing is included
        ...
        Udelta = (np.pi * hs) / (tp * np.sinh(kh))  # peak wave orbital velocity
        fw = np.exp(-5.977 + 5.213 * (Adelta / ksw) ** -0.194)  # wave-related friction coefficient (van Rijn)
        fw = np.min(fw, 0.3)
